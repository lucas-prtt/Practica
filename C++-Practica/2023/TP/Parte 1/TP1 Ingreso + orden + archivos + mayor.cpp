...
{ // Declaracion de variables
    ...
    if (archivoexiste("RepMoto.dat") and archivoexiste("RepAuto.dat") and archivoexiste("RepCamion.dat") and archivoexiste("RepCamioneta.dat"))
    {
        ...
        else
        {
            ...
            // IMPORTANTE: hice esta funcion de abajo para recrear el vector de zonas si se cargan los archivos por si utilizan este vector para los puntos 2/3/4. Si no lo usan borrenlo para evitar gastar CPU
...
void informeporzona(Zona zonas[])
{
    cout << "Informe de cantidad de repartidores por cada zona:" << endl;
    for (int i = 0; i < 14; i++)
        {
        cout << "Zona " << i + 1 << ":" << endl;
...
void zonamayorempleados(Zona zonas[])
{
    ...
    for (int i = 0; i < 14; i++)
        ...
        cout << "La zona con mas repartidores es " << mayor + 1 <<" con "<<v[mayor]<<" repartidores."<< endl;
