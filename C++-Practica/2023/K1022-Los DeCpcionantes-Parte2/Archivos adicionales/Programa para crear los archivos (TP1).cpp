    ...
    else
    {
        do
        {
            do
            {
                ...
                fflush(stdin);
                inicializarnombre(temp.nombreyapellido);
                cin.getline(temp.nombreyapellido, sizeof(temp.nombreyapellido));
            } while (!verificarnombre(temp.nombreyapellido));
            do
            {
            ...
            if (Haylugar(transporte, temp.zona, zonas))
            {
                ...
                do
                {
                    ...
                    cin.clear();
                    cin.ignore(100, '\n');
                    if(!verificarpatente(temp.patente))
                    {cout<<"Patente Invalida, use el formato AA123BB\n";}
                } while (!verificarpatente(temp.patente));
                if (transporte == 1)
                {
                    motos[qmotos] = temp;
                    qmotos++;
...
void informeporzona(Zona zonas[])
{
    ...
    for (int i = 0; i < 14; i++)
    {
        ...
        cout << "  Camionetas: " << zonas[i].camionetas << " repartidor/es" << endl
             << endl;
        getch();
    }
    cout << "\n";
...
void nodisponibles(Empleado motos[], Empleado autos[], Empleado camiones[], Empleado camionetas[])
{
    ...
    if (disponible(motos) < 0)
    {
        pri = true;
        cout << endl
             << "Medios de Transporte no disponibles: " << endl;
        cout << "Moto" << endl;
    }
