//Tras arreglar problemas en el manejo de memoria al intentar agregar un nodo ya existente al arbol
//y ordenar funcion de carga de archivos
...
void cargararchivo(nodorepartidor *&listarepartidores, FILE *archivo, int transporte);
...
void ingresarpedido(colapedidos matrizpedidos[14][4], nodorepartidor *listarepartidores)
{
    ...
    {
        cout << endl
             << "Ingrese el volumen del paquete en metros cubicos: ";
    ...
    if (!presenciaderepartidoresconfirmada(listarepartidores, zona, trans))
    { // Si no hay repartidores
...
void cargarrepartidores(nodorepartidor *&listarepartidores)
{
    ...
    FILE *motos = fopen("RepMoto.dat", "rb");
    FILE *autos = fopen("RepAuto.dat", "rb");
    FILE *camionetas = fopen("RepCamioneta.dat", "rb");
    FILE *camiones = fopen("RepCamion.dat", "rb");
    cout << "Repartidores registrados: \n\n";
    cargararchivo(listarepartidores, motos, 0);
    cargararchivo(listarepartidores, autos, 1);
    cargararchivo(listarepartidores, camionetas, 2);
    cargararchivo(listarepartidores, camiones, 3);
}
void cargararchivo(nodorepartidor *&listarepartidores, FILE *archivo, int transporte)
{
    fseek(archivo, 0, 0);
    reparchivo repartidor;
    fread(&repartidor, sizeof(repartidor), 1, archivo);
    while (!feof(archivo)){
        insertarrepartidor(listarepartidores, repartidor, transporte);
        fread(&repartidor, sizeof(repartidor), 1, archivo);
    }
}
...
{                                           // Quedo medio inutil esta funcion aca
...
void arbolInsertar(NodoArbol *&raiz, infopedido pedidodesencolado)
{
    ...
    else
    {
        ...
        {
            ant = r;
            if (pedidodesencolado.codigo > ant->dato.codigo)
            {
                r = r->izq;
            }
            else
...
void mostrar(nodorepartidor *listarepartidores)
{
    ...
    {
        if (repartidoractual->info.pedidos != NULL)
        { // Si tiene pedidos
...
int ingresoverificadoATOI(int lmax)
{
    ...
    do
    {
        ...
        i = 0;
        while (i < lmax and isdigit(palabra[i]))
        { // Busca ultimo caracter que no es un numero
            i++;
        }
...
float ingresoverificadoATOF(int lmax)
{
    ...
    do
    {
        ...
        yahubounpunto = false;
        i = 0;
        while (i < lmax and (isdigit(palabra[i]) or (!yahubounpunto and palabra[i] == ('.'))))
        { // Busca ultimo caracter que no es un numero
            if (palabra[i] == '.')
                yahubounpunto = true;
            i++;
