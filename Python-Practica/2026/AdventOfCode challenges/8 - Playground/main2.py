...
def connectClosestTogether(boxes:list[JunctionBox], boxesDistances:list[tuple[JunctionBox, JunctionBox, float]]):
    # Posiblemente haya algo mas eficiente. Tarda 11,5s en computar todo. Para esta cantidad de cajas sirve 
    box1 = None
    box2 = None
    while(len(boxes[0].getCircuit()) != len(boxes)):
        box1, box2, _ = boxesDistances.pop(0)
...
startTime = time.perf_counter_ns()
box1, box2 = connectClosestTogether(boxes, boxesDistances)
print("Closest boxes connected in {} ms".format(msSince(startTime)))

circuits = nonRepeatingCircuits(boxes)
circuits.sort(key=len, reverse=True)

print("Circuits: {}, Last boxes connected: {} - {}".format(len(circuits), box1, box2))
print("Answer: {}".format(box1.x * box2.x) )
